...
router.post('/signup', async function (req, res, next) {
    try {
        ...
        User_1.default.findOne({ email: req.body.email }, (err, user) => {
            if (err)
                return res.json("No user error...");
            if (!user) {
                User_1.default.findOne({ phone_number: req.body.phone_number }, async (err, phone_number) => {
                    if (!phone_number) {
...
router.post("/login", async (req, res) => {
    try {
        const { error } = (0, usersValidation_1.validateUserLogin)(req.body); // validate the user
        if (error)
            return res.status(400).json({ error: error.details[0].message }); // throw validation errors
        const user = await User_1.default.findOne({ email: req.body.email });
        if (!user)
            return res.status(400).json({ error: "Email you provided is not correct" }); // log errors if email is incorrect
